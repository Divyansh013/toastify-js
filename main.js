...
async function playCombinedAudio() {
  const combinedBuffer = audioContext.createBuffer(
    1,
    hardcodedAudioBuffer.length + userAudioBuffer.length,
    audioContext.sampleRate
  );

  const output = combinedBuffer.getChannelData(0);

  output.set(hardcodedAudioBuffer.getChannelData(0), 0);

  output.set(userAudioBuffer.getChannelData(0), hardcodedAudioBuffer.length);
...
window.onload = setupAudioContext;
